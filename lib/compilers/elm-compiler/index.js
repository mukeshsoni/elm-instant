...
function getGeneratedMainFileContent (expressions, importStatements, statements, userModuleName) {
  ...
  let fileContent = `module Main exposing (..)
 ...
 */
function getComponentKey (elmCode, index, code) {
...
function getSource (module, expressions, codePath) {
  ...
  cachedSources[timestamp] = global.module.exports[_.capitalize(fileName)]
  // } else {
  ...
  return cachedSources[timestamp]
}
    ...
    .then((elmCode) => {
      return new Promise((resolve, reject) => {
        return promisifiedExec(`cd ${codePath} && elm-make --yes ${previewFileName}.elm --output=${previewFileName}.js`)
          .then(() => {
            let source = getSource(module, elmCode, codePath);
            ...
            let elmComponent = <span>a</span>;

            // only return elm component is source is not corrupted
            // style={{display: 'flex', justifyContent: 'center'}}
            if (source && source.embed) {
                elmComponent = (
                  <div key={getComponentKey(elmCode, index, code)}>
                    <Elm key={getComponentKey(elmCode, index, code)} src={source} />
                  </div>
                )
            }
