        ...
        elmMakePromises = new Promise((resolve, reject, onCancel) => {
          ...
          onCancel(() => {
            ps.lookup({
              ...
              if(err) {
                console.log('error getting command info', err.toString())
              } else {
                resultList.forEach((process) => {

                  ps.kill(process.pid, (err) => {
                    if(err) {
                      console.error('Error killing process ', err.toString())
                    } else {
                      console.log('killed elm-make process with id', process.pid)
                    }
                  })
                })
              }
