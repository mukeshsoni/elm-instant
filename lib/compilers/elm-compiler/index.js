...
function writeFilesForExpressions(playgroundCode, userModuleName) {
  ...
  const frolicExpressions = tokenizedCode.filter(code => code.type === 'frolic')
  ...
  const frolicFileWritePromises = frolicExpressions.map((expression, index) =>
    writeFile(`${codePath}/main${index + expressions.length}.elm`, getGeneratedFrolicFileContent(expression, importStatements, statements, userModuleName, index + expressions.length))
  )

  return Promise.all(fileWritePromises.concat(frolicFileWritePromises)).then(() => expressions.concat(frolicExpressions))
}
...
function getComponentKey(expressions, index, code, prefix = '') {
  ...
  if (cachedCode.trim() !== code.trim() || !cachedComponentKeys[cacheKey]) {
