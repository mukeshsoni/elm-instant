...
previewItems = [ `
    ...
    fileContent += expression.commands.map(function(command) {
        let result = ''
        const cleanExpression = cleanUpExpression(command.value)
        const newLinesBefore = command.newLinesBefore
        const newLinesAfter = !isNaN(command.newLinesAfter) ? command.newLinesAfter : 0
        if (newLinesBefore > 0) {
            if (totalCommands > 0) {
                result += `    , EmptyLines ${newLinesBefore}
`
            } else {
                result += `      EmptyLines ${newLinesBefore}
`
        ...
        totalCommands++
        result += ((totalCommands > 1) || (newLinesBefore > 0))
            ? `    , UserExpression (toString (${cleanExpression}))`
            : `      UserExpression (toString (${cleanExpression}))`

        if (newLinesAfter && (newLinesAfter > 0)) {
            result += `    , EmptyLines ${newLinesAfter}`
        }
...
function writeFilesForExpressions (playgroundCode, userModuleName, codePath) {
  ...
  let expressionsWithLinesData = expressions.map((expression, exprIndex) => {
      let expressionClone = Object.create(expression)
      if (expression.commands) {
          expressionClone.commands =
              expression.commands.filter(function(command) {
                  return (command.value.trim() !== "")
              }).map(function(command, cmdIndex) {
                  let commandClone = Object.create(command)
                  commandClone.newLinesBefore = command.lineNumber - lastLineNumber - 1
                  lastLineNumber = command.lineNumber
                  return commandClone
              });
          let lastCommand = expressionClone.commands[expressionClone.commands.length - 1]
          const nextLineNumber = (exprIndex < (expressions.length - 1))
              ? expressions[exprIndex + 1].lineNumber
              : undefined
          lastCommand.newLinesAfter = nextLineNumber - lastCommand.lineNumber - 1
      } else {
  ...
  const fileWritePromises = expressionsWithLinesData.map((expression, index) => {
  ...
  return Promise.all(fileWritePromises).then(() => expressionsWithLinesData)
}
...
function getComponentKey (expressions, index, code, prefix = '') {
  if (cachedCode.trim() !== code.trim() || !cachedComponentKeys[getExpressionValue(expressions[index]) + index]) {
    cachedComponentKeys[getExpressionValue(expressions[index]) + index] = prefix + Math.floor(Math.random() * 10000) + '_' + index
  } else {
  ...
  if (expressions[index].type !== 'renderExpression') {
    cachedComponentKeys[getExpressionValue(expressions[index]) + index] = cachedComponentKeys[getExpressionValue(expressions[index]) + index] + Math.random() * 1000
  }
...
function getSource (module, expression, index, codePath) {
  ...
  try {
    ...
    cachedNonEvaledFiles[getExpressionValue(expression)] = fs.readFileSync(`${codePath}/${fileName}.js`).toString()
    // }
    eval(cachedNonEvaledFiles[getExpressionValue(expression)])
  } catch (e) {
          ...
          .then(() => {
            ...
            const elmComponents = sources.map((source, index) => {
              ...
              if (source && source.embed) {
                return (
                  <div key={getComponentKey(expressions, index, code, openFilePath)}>
                    <Elm key={getComponentKey(expressions, index, code, openFilePath)} src={source} />
                  </div>
                )
              } else {
