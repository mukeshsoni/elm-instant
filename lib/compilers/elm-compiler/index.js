...
main =
    ...
    lastUserLine = expression.lineNumber
  } else {
    fileContent = `module Main${counter} exposing (..)
...
previewItems = [ `
    ...
    fileContent += expression.commands
                .filter(function(command) {
                    return (command.value.trim() !== "")
                })
                .map(function(command) {
                    let result = ''
                    let newLinesBefore = (command.lineNumber - lastUserLine) - 1
                    if (newLinesBefore > 0) {
                        if (totalCommands > 0) {
...
`
                    ...
                    let cleanExpression = cleanUpExpression(command.value)
                    console.log(cleanExpression, command.newlines, command)
                    totalCommands++
                    result += ((totalCommands > 1) || (newLinesBefore > 0))
                        ? `    , UserExpression (toString (${cleanExpression}))`
                        : `      UserExpression (toString (${cleanExpression}))`;
                    lastUserLine = command.lineNumber
                    return result
                }).join('\n')
