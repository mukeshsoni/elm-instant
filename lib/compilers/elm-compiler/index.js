...
previewItems = [ `
    ...
    fileContent += expression.commands.map((command) => {
      let result = ''
...
function writeFilesForExpressions(playgroundCode, userModuleName) {
  ...
  const expressionsWithLinesData = expressions.map((expression, exprIndex) => {
    const expressionClone = Object.create(expression)
    if (expression.commands) {
      expressionClone.commands = expression.commands.filter((command) => (command.value.trim() !== ''))
      .map((command) => {
        const commandClone = Object.create(command)
        commandClone.newLinesBefore = command.lineNumber - lastLineNumber - 1
        lastLineNumber = command.lineNumber
        return commandClone
      })
      const lastCommand = expressionClone.commands[expressionClone.commands.length - 1]
...
function getSource(module, expression, index) {
  ...
  try {
    ...
    console.log('error evaluating bundle', e.toString()) // eslint-disable-line no-console
          ...
          .then(() => {
            ...
            const elmComponents = sources.map((source, index) => {
              ...
              if (source && source.embed) {
                ...
                return <span>{Empty}</span> // eslint-disable-line
              }
            })
